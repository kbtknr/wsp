...
function generateNotFoundRoute(site) {
  if (!site.paths.notFoundFile) {
    return null;
  }

  const notFoundExt = path.extname(site.paths.notFoundFile);
  return generateRouteCode({
    urlPath: '*',
    componentPath: `@internal/not-found${notFoundExt}`,
  });
}
function generateRoutes(site) {
  ...
  const notFoundRouteCode = generateNotFoundRoute(site);

  return `
    export default [
      ${[routesCode, notFoundRouteCode].join(',')}
    ];
  `;
}
