...
const MarkdownItCustomBlockContainer = require('./markdown-it-custom-block-container');
const MarkdownItCustomInlineContainer = require('./markdown-it-custom-inline-container');
...
module.exports.renderMarkdown = function(config, content) {
  ...
  const md = MarkdownIt(mdconf)
    ...
    .use(MarkdownItSup)
    .use(MarkdownItIns)
    .use(MarkdownItCustomBlockContainer, {
      loaders: config.customContainerLoaders,
    })
    .use(MarkdownItCustomInlineContainer, {
      loaders: config.customContainerLoaders,
    });
  ...
  (md => {
    md.renderer.rules.html_block = (tokens, idx) => {
      ...
      const reScriptStyle = /^\s*<(script|style)(?=(\s|>|$))/i;
      if (reScriptStyle.test(content)) {
