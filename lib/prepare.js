...
module.exports = async function(siteConfigPath, tmpDir) {
  ...
  const defaultMasterPagePath = site.config.defaultMasterPageName
    ? `${site.paths.masterPagesDir}/${site.config.defaultMasterPageName}.vue`
    : `${path.resolve(__dirname, '../app/default-master-page.vue')}`;
  await fs.symlink(defaultMasterPagePath, path.resolve(site.internalDir, 'default-master-page.vue'));
  ...
  const defaultMarkdownWrapperPath = site.config.defaultMarkdownWrapperName
    ? `${site.paths.componentsDir}/${site.config.defaultMarkdownWrapperName}.vue`
    : `${path.resolve(__dirname, '../app/default-markdown-wrapper.vue')}`;
  await fs.symlink(defaultMarkdownWrapperPath, path.resolve(site.internalDir, 'default-markdown-wrapper.vue'));
