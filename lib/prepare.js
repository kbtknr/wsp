...
module.exports = async function(siteConfigPath, tmpDir) {
  ...
  await wsdutil.fsWriteFile(routesPath, generateRoutes(site));

  const masterPagesPath = `${site.tmpDir}/master-pages.js`;
  await wsdutil.fsWriteFile(masterPagesPath, generateMasterPages(site));
