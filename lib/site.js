...
class Site extends EventEmitter {
  ...
  static async create(configFilePath, config) {
    await SiteConfig.create(configFilePath, config);
  }
  ...
  load() {
    this.config.load(this.siteConfigPath);
  }
  ...
  async getPage(pathName) {
    return await this.pages.get(pathName);
  }
  async createPage(pathName, src) {
    return await this.pages.create(pathName, src);
  }
  async updatePage(pathName, src) {
    return await this.pages.update(pathName, src);
  }
  async deletePage(pathName) {
    return await this.pages.delete(pathName);
  }
  ...
  async readyBuild() {
    ...
    await this.generateSiteDataFile(path.join(this.tmpDir, 'site-data.js'));
  ...
  async unreadyBuild() {
    ...
    await fs.unlink(path.join(this.tmpDir, 'site-data.js'));
    this.tmpDir = null;
  }
  ...
  generateSiteDataFileGoogleAnalytics(siteConfig) {
    const siteConfigGoogleAnalytics = siteConfig.googleAnalytics;
    if (!siteConfigGoogleAnalytics) {
      return 'null';
    }

    const trackingID =
      typeof siteConfigGoogleAnalytics.trackingID === 'string' ? siteConfigGoogleAnalytics.trackingID : null;
    return `
      {
  ...
  async generateSiteDataFile(filename) {
    const siteConfig = this.config;
    const siteDataCode = `
module.exports = {
  siteName: ${JSON.stringify(siteConfig.siteName)},
  siteTitle: ${JSON.stringify(siteConfig.siteTitle)},
  siteDescription: ${JSON.stringify(siteConfig.siteDescription)},

  googleAnalytics: ${this.generateSiteDataFileGoogleAnalytics(siteConfig)}
};
    `;
    await fs.writeFile(filename, siteDataCode);
  }
  ...
  createBaseConfig(isServer, isProd, isDebug, entryFilePath, outDir) {
    ...
    config.resolve
      ...
      .set(siteDataAlias, path.join(this.tmpDir, 'site-data.js'))
    ...
    config.module
      ...
      .test(/\.(png|jpe?g|gif)(\?.*)?$/)
      .oneOf('responsive')
        .resourceQuery(/sizes|size|quality|format|placeholder|placeholderSize/)
        .use('responsive-loader')
          .loader('responsive-loader')
          .options({
            name: `_assets/rimg/[name].[hash:8].[width].[ext]`
          })
          .end()
        .end()
      .oneOf('original')
  ...
  async build(outDir, isDebug = false) {
    ...
    const viewportContent = this.config.getViewportContent();
    const viewportTag = viewportContent == null ? '' : `<meta name="viewport" content="${escape(viewportContent)}">`;

    await this.renderPages(renderer, viewportTag, '/', outDir);
  }
  async renderPages(renderer, viewportTag, urlDir, outDir) {
    const nodes = await this.listPages(urlDir);
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (node.isDirectory) {
        await this.renderPages(renderer, viewportTag, path.join(urlDir, name), path.join(outDir, name));
      } else {
        const extname = path.extname(name);
        const basename = path.basename(name, extname);
        ...
        try {
          await this.renderPage(renderer, viewportTag, pathname, outFile);
        } catch (e) {
  ...
  async renderPage(renderer, viewportTag, pathname, outFile) {
    const context = {
      url: pathname,
      headData: {},
    ...
    clientConfig.plugin('html').use(require('html-webpack-plugin'), [
      {
        ...
        meta: {
          viewport: this.config.getViewportContent(),
        },
      },
    ]);
