...
class Site extends EventEmitter {
  ...
  async listPages(pathName) {
    if (!path.isAbsolute(pathName)) {
      throw new Error("pathName is required absolute path");
    }

    const fsDirName = path.join(this.config.pagesDir, pathName.substring(1));
    const names = await fs.readdir(fsDirName);
    const entries = [];
    for (let i = 0; i < names.length; i++) {
      const name = names[i];
      const fsPathName = path.join(fsDirName, name);
      const stats = await fs.stat(fsPathName);
  ...
  async listMasterPages(pathName) {
    const masterPagesDir = this.config.masterPagesDir;
    const names = await fs.readdir(masterPagesDir);
    const entries = [];
    for (let i = 0; i < names.length; i++) {
      const name = names[i];
      const ext = path.extname(name);
      ...
      const stats = await fs.stat(path.join(masterPagesDir, name))
      if (!stats.isFile()) {
        continue;
      }
  ...
  async readyBuild() {
    ...
    await this.ensureSymlink(this.config.defaultMasterPageFile, path.join(this.tmpDir, 'default-master-page.vue'));
    await this.ensureSymlink(this.config.defaultMarkdownWrapperFile, path.join(this.tmpDir, 'default-markdown-wrapper.vue'));
  }
  async unreadyBuild() {
    if (this.tmpDir == null) {
      return;
  ...
  async generateRoutesCode(urlName, urlDir, importDir) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (node.isDirectory) {
        routesCode.push(
          await this.generateRoutesCode(name, path.join(urlDir, name), path.join(importDir, name))
        );
      } else {
  ...
  createBaseConfig(isServer, isProd, isDebug, entryFilePath, outDir) {
    ...
    config.resolve
      ...
      .alias.set(
        headMixinAlias,
        path.join(appDir, isServer ? 'head-mixin-server.js' : 'head-mixin-client.js')
      )
    ...
    config.module
      ...
      .use('head-loader')
      .loader(require.resolve(path.join(libDir, 'head-loader')));
    ...
    mdRule
      .use('markdown-loader')
      .loader(require.resolve(path.join(libDir, './markdown-loader')))
      .options({
  ...
  createClientConfig(isProd, isDebug, outDir) {
    ...
    if (isProd) {
      ...
      config.plugin('ssr-client').use(require(path.join(libDir, './ClientPlugin')), [
        {
