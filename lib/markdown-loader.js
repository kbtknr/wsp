...
function generateComponentCode(component) {
  const key = component;
  const requirePath = `@internal/components/${component}`;
  return `${JSON.stringify(key)}: require(${JSON.stringify(requirePath)}).default`;
}
function generateComponentsCode(components) {
  ...
  return `${JSON.stringify('components')}: {` + components.map(com => generateComponentCode(com)).join(',') + '}';
}
...
module.exports = function(src) {
  ...
  return (
    ...
    `<script>` +
    `export default {` +
    `${componentsCode == null ? '' : componentsCode}` +
    `}` +
    `</script>` +
